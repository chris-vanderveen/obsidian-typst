...
export default class ExcalidrawPlugin {
  ...
  async addTypst(code: string, processor: ExcalidrawProcessor) {
    if (!this.ea)
      // @ts-expect-error
      this.ea = this.plugin.app.plugins.plugins["obsidian-excalidraw-plugin"]
    ...
    try {
      ...
      const svg = (
        await this.plugin.typst.svg(code, "excalidraw", processor.id)
      ).svg;
      ...
      const dataurl = await this.ea.convertStringToDataURL(
        svg,
        "image/svg+xml",
      );
      ...
      this.ea.elementsDict[id] = this.ea.boxedElement(
        id,
        "image",
        pos.x,
        pos.y,
