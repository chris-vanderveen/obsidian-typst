...
export default class SnippetSuggestElement extends HTMLElement {
  ...
  suggest(query: string, cursorPos: EditorPosition, argument?: string) {
    this.candidates =
      this.plugin.settings.snippets?.filter((s) => s.name.includes(query)) ??
      [];
  ...
  private render(position: PopupPosition) {
    ...
    this.candidates.forEach((snippet, index) => {
      ...
      if (snippet.script) {
        ...
        item.appendChild(
          document.createTextNode(`${snippet.name} (${snippet.category})`),
        );
  ...
  onKeyDown(e: KeyboardEvent) {
    ...
    switch (e.key) {
      ...
      case "Enter": {
        ...
        let snippet: Snippet;
        if (this.selectedIndex >= 0)
          snippet = this.candidates[this.selectedIndex]! ?? this.candidates[0]!;
        else snippet = this.candidates[0]!;

        if (snippet.script && this.argument === undefined)
  ...
  private execute(snippet: Snippet) {
    ...
    if (snippet.script) {
      try {
        content = new Function("input", "window", content)(
          this.argument?.slice(1, -1),
          window,
        );
      } catch (e) {
        new Notice(String(e));
    ...
    const newCursorPos = {
      ...
      ch:
        this.queryPos!.ch + (cursorIndex === -1 ? content.length : cursorIndex),
    };
